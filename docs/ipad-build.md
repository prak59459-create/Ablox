...
- **`Ablox.swiftpm/Package.swift`**, because it imports `AppleProductTypes`,
...
The drawn Ablox mark lives in `design/AppIcon.png`; set it from Swift
...
reported three separate problems.
...
| Build | Module name |
|---|---|
| Swift Playgrounds on device | `AbloxApp` (the executable target) |
